...
import 'package:app_historial_vehiculo/screens/Gastos/expense_detail_screen.dart'; // Para volver al detalle del gasto (si aplica)
...
class ChangesSavedScreen extends StatelessWidget {
  final Map<String, dynamic>? expense; // Hacemos expense opcional
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            logger.i('Volver desde Cambios Guardados (AppBar)');
            // Decidir a dónde volver: al detalle si hay un gasto, o a la lista de gastos
            if (expense != null && expense!['id'] != null) {
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => ExpenseDetailScreen(expense: expense!)), // Volver al detalle
                (Route<dynamic> route) => false,
              );
            } else {
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => const ExpensesScreen()), // Volver a la lista general
                (Route<dynamic> route) => false,
              );
            }
          },
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                logger.i('Botón Volver a Gastos presionado');
                // Navegar a la pantalla de la lista de gastos y limpiar la pila
                Navigator.pushAndRemoveUntil(
                  context,
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          logger.i('Navegación inferior en ChangesSavedScreen: $index');
